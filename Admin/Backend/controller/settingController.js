...
const addGlobalSetting = async (req, res) => {
  try {
    ...
    await flacksTable.updateOne({tableName:"Settings"},{$set : {status : 1}})
    ...
    const globalSetting = await Setting.updateOne(
      ...
      },
      {
        $set: {
          ...
          "setting.ReferalAmount":req.body.setting.ReferalAmount,
          "setting.refferalwithdraw":req.body.setting.refferalwithdraw,
          "setting.refferalshopping":req.body.setting.refferalshopping
        },
      }
    );
    await flacksTable.updateOne({tableName:"Settings"},{$set : {status : 1}})
