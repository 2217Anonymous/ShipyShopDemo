...
const OrderServices = {
  getAllOrders: async ({
    ...
    let formdata = {customerName:searchName,status,day:searchDay,page:page,limit:limit,startDate :startD,endDate :endD}
    return requests.post(
      `orders/orderView`,formdata,
      body,
      headers
    );
  },
  ...
  ViewSingleorder: async (body) => {
    return requests.post("/orders/orderSingleView",body);
  },

  StatusChanges: async (body) => {
    return requests.post("/orders/orderStatusUpdation",body);
  },
};
