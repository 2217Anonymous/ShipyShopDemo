...
const ProductServices = {
  getAllProducts: async ({ page, limit, category, title, price }) => {
    ...
    if(searchTitle !== ''){
      return requests.get(`/products?category=${searchCategory}&title=${searchTitle}&price=${searchPrice}`);
    }else{
      return requests.get(`/products?category=${searchCategory}&title=${searchTitle}&price=${searchPrice}`);
    }
  },
  ...
  updateProduct: async (id, body,updateId) => {
    return requests.post(`/products/updateProduct/${id}`, body,{updateId});
  },
