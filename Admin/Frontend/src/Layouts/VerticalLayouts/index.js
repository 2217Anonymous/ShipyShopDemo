...
const VerticalLayout = (props) => {
    const [navData, setNavData] = useState(navdata().props.children)
    /*
 layout settings
 */
    ...
    const handleClick = (key, event) => {
        ...
        const nav1 = [...navData]
        const nav2 = JSON.parse(JSON.stringify(nav1))
        const nav = nav2.map((item, index) => {
            return {
                ...item,
                IsOpen: index === key ? !item.IsOpen : false,
            };
        });
        setNavData(nav)
    }
    ...
    //     if (item.subItems) {
    //         const filteredSubItems = item.subItems.filter(subItem => !subItem.role || subItem.role === role);
                            ...
                            : (
                                (item.subItems ? (
                                                    ...
                                                    <React.Fragment key={key}>
                                                        {!subItem.isChildItem ? (
                                                            ...
                                                            <li className="nav-item">
                                                                ...
                                                                <Collapse className="menu-dropdown" isOpen={subItem.IsOpen} id="sidebarEcommerce">
                                                                                ...
                                                                                <React.Fragment key={key}>
                                                                                    {!childItem.childItems ?
                                                                                        ...
                                                                                        : <li className="nav-item">
                                                                                            ...
                                                                                            <Collapse className="menu-dropdown" isOpen={childItem.IsOpen} id="sidebaremailTemplates">
                                                                                                <ul className="nav nav-sm flex-column">
                                    ...
                                    <li className="nav-item">
                                        <Link
                                            className="nav-link menu-link"
                                            onClick={(event) => handleClick1(key)}
                                            to={item.link ? item.link : "/#"}>
